...
# & or , works for adding multiple conditions of filter function
...
NTL.phys.data.water.temp <- 
  NTL.phys.data.pipe.Tues %>%
  filter(depth == 0) %>%
  summarize(mean_surface_temp = mean(temperature_C))

class(NTL.phys.data.water.temp)
...
NTL.phys.data.temp.Peter.Paul <- 
  NTL.phys.data.processed %>%
  filter(depth == 0) %>%
  drop_na(temperature_C) %>%
  group_by(lakename) %>%
  summarize(surface_mean = mean(temperature_C))
